...
def result_final_enrichment(
    ...
    for res in results:
        ...
        if hasattr(res, 'address') and isinstance(getattr(res, 'address'), AddressInfo):
            ...
            if getattr(res, 'address').district is None:
                res.address.district = pipeline.default_district(driver, datum)
        ...
        if page_title:
            if hasattr(res, 'category') and not res.category:
                res.category = page_title
        if group_title:
            if hasattr(res, 'subcategory') and not res.subcategory:
